...
static LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam){
    ...
    case WM_CREATE:{
        ...
        /* second row: ARM support */
        CreateWindowExA(0, "BUTTON", "ENABLE ARM (1-time)", WS_CHILD | WS_VISIBLE, 10, 124, 170, 32, hwnd, (HMENU)IDC_BTN_ARM, g_hInst, NULL);
        CreateWindowExA(0, "STATIC", "Runs 32-bit ARM APKs (needs Android running once). ARM64 = impossible on any PC emulator.",
            WS_CHILD | WS_VISIBLE | SS_LEFT, 190, 131, 360, 20, hwnd, NULL, g_hInst, NULL);
        SendMessageA(GetDlgItem(hwnd, IDC_BTN_ARM), WM_SETFONT, (WPARAM)fSmall, TRUE);
        SendMessageA(GetDlgItem(hwnd, 2001), WM_SETFONT, (WPARAM)fSmall, TRUE);
        ...
        LogF("ARM64-only apps cannot run on ANY Windows emulator (even BlueStacks).");
        return 0;
    }
